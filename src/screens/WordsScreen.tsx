...
export function WordsScreen() {
  ...
  const [swipeViewportWidth, setSwipeViewportWidth] = useState(width);
  const previousWidth = useRef(width);
  const swipeTranslateX = useRef(new Animated.Value(0)).current;
  const panelWidth = Math.max(swipeViewportWidth, 1);
  const activeTabOffset = activeTab === "jlpt" ? 0 : -panelWidth;

  const animateSwipeBack = useCallback(() => {
    Animated.spring(swipeTranslateX, {
      toValue: activeTabOffset,
      useNativeDriver: true,
      damping: 18,
      stiffness: 220,
      mass: 0.9
    }).start();
  }, [activeTabOffset, swipeTranslateX]);

  const animateTabSwitch = useCallback(
    (nextTab: WordTab) => {
      const nextOffset = nextTab === "jlpt" ? 0 : -panelWidth;
  ...
  const panResponder = useMemo(
    () =>
      PanResponder.create({
        ...
        onPanResponderMove: (_, gestureState) => {
          const isOverscrollingJlpt = activeTab === "jlpt" && gestureState.dx > 0;
          const isOverscrollingCustom = activeTab === "custom" && gestureState.dx < 0;
          const resistance = isOverscrollingJlpt || isOverscrollingCustom ? 0.18 : 0.35;
          swipeTranslateX.setValue(activeTabOffset + gestureState.dx * resistance);
        },
    ...
    [activeTab, activeTabOffset, animateSwipeBack, animateTabSwitch, swipeTranslateX]
  );
  ...
  useEffect(() => {
    ...
    swipeTranslateX.setValue(activeTab === "jlpt" ? 0 : -panelWidth);
  }, [activeTab, panelWidth, swipeTranslateX, width]);

  const renderJlptPanel = () => (
    <>
                    ...
                    <View style={styles.headerActions}>
                      ...
                      >
                        <AppIcon
                          ...
                          size={18}
                          color={
                            isTtsEnabled ? colors.primaryDeep : colors.textFaint
                          }
                        />
                      </Pressable>
                      <Pressable
      ...
      <View {...panResponder.panHandlers} style={styles.swipeArea}>
        ...
        <View
          ...
          onLayout={(event) => {
            ...
            if (nextWidth > 0 && nextWidth !== swipeViewportWidth) {
              setSwipeViewportWidth(nextWidth);
            }
          }}
        >
          <Animated.View
          ...
          >
            <View style={[styles.swipePanel, { width: panelWidth }]}>
              {renderJlptPanel()}
            </View>
            <View style={[styles.swipePanel, { width: panelWidth }]}>
              {renderCustomPanel()}
