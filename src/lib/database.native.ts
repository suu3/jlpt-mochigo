...
export const quizDatabase = {
  async initialize() {
    ...
    setupPromise = (async () => {
      ...
      await db.execAsync(`
        ...
        CREATE TABLE IF NOT EXISTS word_entries_v2 (
          id TEXT NOT NULL,
          level TEXT NOT NULL,
          lang TEXT NOT NULL,
        ...
        CREATE INDEX IF NOT EXISTS idx_word_entries_v2_level_lang
        ON word_entries_v2 (level, lang);
      `);
    })();
  ...
  async getLevelWords(level: JLPTLevel, language: AppLanguage) {
    const cachedWords = await this.getCachedLevelWords(level, language);
  ...
  async clearWordCache() {
    await this.initialize();
    const db = await getDatabase();
    await db.runAsync("DELETE FROM word_entries_v2");
  }
};
